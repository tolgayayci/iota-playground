...
import { ModuleCall } from '@/lib/types';
...
export function ModuleExecutionHistory({ projectId }: ModuleExecutionHistoryProps) {
  const [calls, setCalls] = useState<ModuleCall[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [sortOrder, setSortOrder] = useState<'desc' | 'asc'>('desc');
  const [selectedCall, setSelectedCall] = useState<ModuleCall | null>(null);
  ...
  const fetchCalls = async () => {
    try {
      ...
      console.error('Error fetching module calls:', error);
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="h-full flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center bg-muted/40">
          <div className="text-center">
            ...
            <h3 className="font-medium mb-3">No Module Interactions</h3>
            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">
                Start interacting with your deployed module
              </p>
              <p className="text-sm text-muted-foreground">
              ...
              <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    window.open(
                      `https://explorer.iota.org/txblock/${call.contract_address}?network=testnet`,
                      '_blank'
                    );
                  }}
                >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            window.open(
                              `https://explorer.iota.org/txblock/${selectedCall.contract_address}?network=testnet`,
                              '_blank'
                            );
                          }}
                        >
