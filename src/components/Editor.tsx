...
import { moveDiagnosticProvider } from '@/lib/languages/moveDiagnostics';
...
export function Editor({ 
  ...
  const handleEditorDidMount = (editor: any, monaco: any) => {
    ...
    // Set the editor instance for the diagnostic provider
    moveDiagnosticProvider.setEditor(editor);
  };
  ...
  // Apply compiler diagnostics when compilation results change
  useEffect(() => {
    if (lastCompilation && editorRef.current && monacoRef.current) {
      const model = editorRef.current.getModel();
      if (model) {
        // Apply compiler errors if compilation failed
        if (!lastCompilation.success && lastCompilation.stderr) {
          moveDiagnosticProvider.applyCompilerDiagnostics(model, lastCompilation.stderr);
        } else if (lastCompilation.success) {
          // Clear compiler errors on successful compilation
          monacoRef.current.editor.setModelMarkers(model, 'move-compiler', []);
        }
      }
    }
  }, [lastCompilation]);
