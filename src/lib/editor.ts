...
import { registerMoveCompletionProvider, registerMoveHoverProvider } from './languages/moveCompletions';
import { moveDiagnosticProvider } from './languages/moveDiagnostics';
import { moveSignatureHelpProvider } from './languages/moveSignatureHelp';
import { moveCodeActionsProvider } from './languages/moveCodeActions';
...
// Initialize Monaco editor with Move language support
export function initializeMonaco(monaco: typeof import('monaco-editor')) {
  // Register Move language
  registerMoveLanguage(monaco);
  registerMoveCompletionProvider(monaco);
  registerMoveHoverProvider(monaco);
  
  // Register advanced Move providers
  monaco.languages.registerSignatureHelpProvider('move', moveSignatureHelpProvider);
  monaco.languages.registerCodeActionProvider('move', moveCodeActionsProvider);
  
  // Set up diagnostic provider for real-time error detection
  let diagnosticInterval: any = null;
  monaco.editor.onDidCreateModel((model) => {
    if (model.getLanguageId() === 'move') {
      // Initial validation
      moveDiagnosticProvider.updateDiagnostics(model);
      
      // Set up real-time validation
      model.onDidChangeContent(() => {
        // Debounce the validation
        if (diagnosticInterval) {
          clearTimeout(diagnosticInterval);
        }
        diagnosticInterval = setTimeout(() => {
          moveDiagnosticProvider.updateDiagnostics(model);
        }, 500);
      });
    }
  ...
  // Keep Rust for backwards compatibility
...
export function defineEditorTheme(monaco: typeof import('monaco-editor'), isDark: boolean) {
  monaco.editor.defineTheme('custom-theme', {
    ...
    rules: [
      ...
      { token: 'type.parameter', foreground: isDark ? 'FFB86C' : 'D16969' },
      { token: 'string', foreground: isDark ? 'A9DC76' : '098658' },
      { token: 'string.escape', foreground: isDark ? 'F1FA8C' : '811F3F' },
      { token: 'number', foreground: isDark ? 'AB9DF2' : '098658' },
      { token: 'number.hex', foreground: isDark ? 'BD93F9' : '5A5CAD' },
      { token: 'comment', foreground: isDark ? '727072' : '008000', fontStyle: 'italic' },
      { token: 'annotation', foreground: isDark ? 'FC9867' : 'AF00DB', fontStyle: 'italic' },
      { token: 'namespace', foreground: isDark ? '8BE9FD' : '267F99' },
      { token: 'predefined', foreground: isDark ? '50FA7B' : '36AC46' },
      ...
      { token: 'delimiter', foreground: isDark ? 'F8F8F2' : '000000' },
    ],
    colors: {
