...
import { FileCode2, Terminal, PlayCircle, Wand2, Clock, Calendar, Pencil, Check, X, Bug, Zap } from 'lucide-react';
...
const VIEWS = [
  ...
  { id: 'abi', title: 'Module Interface', icon: PlayCircle },
  { id: 'ptb', title: 'PTB Builder', icon: Zap },
...
export function EditorPage() {
  ...
  const [refreshModuleTrigger, setRefreshModuleTrigger] = useState(0);
  ...
  const handleCompile = async () => {
    ...
    try {
      ...
      // Update project activity and store compilation result
      await supabase
        .from('projects')
        .update({ 
          last_activity_at: new Date().toISOString(),
          updated_at: new Date().toISOString(),
          last_compilation_result: result.success ? result : null,
        })
        .eq('id', project.id);
  ...
  const handleDeploySuccess = () => {
    ...
    setRefreshModuleTrigger(prev => prev + 1);
  };
  ...
  const hasPTB = activeViews.includes('ptb');
  const mainHeight = hasConsole ? 'h-[75%]' : 'h-full';
  const consoleHeight = 'h-[25%]';

  const getMainPanelWidth = () => {
    const activeMainViews = [hasEditor, hasABI, hasPTB].filter(Boolean).length;
